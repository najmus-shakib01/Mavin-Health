    ...
    (userMessage) => {
      ...
      if (stageForPrompt === 1) {
        ...
        if (isEnglish) {
          ...
          "Thank you for sharing that you have ${condition || "this health concern"
            }. <br><br> To help you better, please tell me your ${missingText}."`;
        }

        const missingTextAr = joinWithAnd(missingAr, "و", "، ");
        return `المستخدم شارك أعراضه الأولية المتعلقة بـ ${condition || "حالة طبية"
          }. مهمتك الوحيدة في هذا الرد هي جمع **${missingTextAr}**.
      ...
      if (stageForPrompt === 2) {
        return isEnglish
          ? `The user has already provided their basic information (age, gender, and duration) for ${condition || "their medical condition"}. Now your ONLY task is to ask them to describe their **symptoms in detail**.

              Keep the reply focused on collecting symptom details (what they feel, where, since when, what makes it better or worse) without giving medical explanations yet.

              Create a concise response with condition‑specific examples. For example, if they mentioned diabetes:
              "Thank you for providing the necessary information. <br><br> Now please share your **symptoms in detail**. For example — if you're talking about diabetes, you can write:
              'I have had diabetes for 3 years, my blood sugar levels are often high in the morning, I feel thirsty frequently, and I've been experiencing blurred vision lately.'"` : `قدّم المستخدم بالفعل معلوماته الأساسية (العمر والجنس ومدة المشكلة) لـ ${condition || "حالته الطبية"
          }. مهمتك الآن هي طلب **وصف الأعراض بالتفصيل فقط**.

            ركّز في الرد على طلب تفاصيل الأعراض (ماذا تشعر بالضبط، وأين في الجسم، ومنذ متى، وما الذي يحسن أو يزيد الأعراض) دون تقديم تفسيرات طبية أو تشخيص في هذه المرحلة.

            قم بإنشاء رد موجز مع أمثلة خاصة بالحالة. على سبيل المثال، إذا ذكروا السكري:
            "شكرًا لتقديم المعلومات الضرورية. <br><br> الآن يرجى مشاركة **أعراضك بالتفصيل**. على سبيل المثال — إذا كنت تتحدث عن السكري، يمكنك كتابة:
            'لدي السكري منذ 3 سنوات، مستويات السكر في الدم غالبًا ما تكون مرتفعة في الصباح، أشعر بالعطش كثيرًا، وقد كنت أعاني من ضعف الرؤية مؤخرًا.'"`;
      }
